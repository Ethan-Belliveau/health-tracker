// POST /api/sync - saves full app state to Upstash Redis for cross-device backup.
  ...
  try {
    ...
    const backup = {
      state: {
        ...
        hiddenMeals: Array.isArray(state.hiddenMeals) ? state.hiddenMeals : [],
        groceryChecked: Array.isArray(state.groceryChecked) ? state.groceryChecked : [],
        dailyDraft: state.dailyDraft && typeof state.dailyDraft === 'object' ? state.dailyDraft : null,
        updatedAt: state.updatedAt || new Date().toISOString()
      },
      syncedAt: new Date().toISOString()
    };

    await redis.set(BACKUP_KEY, JSON.stringify(backup), { ex: TTL_SECONDS });
    await redis.set(LEGACY_LOG_KEY, JSON.stringify({ log: backup.state.log, syncedAt: backup.syncedAt }), { ex: TTL_SECONDS });
