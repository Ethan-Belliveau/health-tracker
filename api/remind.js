...
export default async function handler(req, res) {
  ...
  try {
    const ids = await kv.smembers('push:ids');
    ...
    await Promise.all(ids.map(async id => {
      try {
        const raw = await kv.get(`push:${id}`);
        if (!raw) { await kv.srem('push:ids', id); return; }
        ...
        if (err.statusCode === 410 || err.statusCode === 404) {
          await kv.del(`push:${id}`);
          await kv.srem('push:ids', id);
        }
